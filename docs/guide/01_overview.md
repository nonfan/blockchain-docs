...
| 项目         | 2009 年原始代码 (v0.1.0)                  | 2025 年现行 Bitcoin Core (v28.x)                                      | 是否仍是原始 |
|--------------|-------------------------------------------|------------------------------------------------------------------------|--------------|
| 共识规则     | 1MB 区块上限、SHA-256 PoW、21M 总量      | 仍保留，但增加了 SegWit（2017）、Taproot（2021）等软分叉升级          | 部分保留     |
| 区块大小     | 严格 1MB                                  | 实际可达 ~4MB（含 witness data）                                      | 已改变       |
| 签名算法     | 只能裸露公钥 + ECDSA                      | 支持 Schnorr 签名（Taproot）、遗留 P2PKH/P2SH、SegWit、Taproot 多种格式 | 完全不同     |
| 脚本系统     | 非常基础，只有少数 opcode 可用            | 加入大量新 opcode（如 OP_CHECKSIGADD）、MiniScript、Covenants 提案准备 | 完全不同     |
| P2P 网络协议 | 极其简单，几十行代码                      | Erlay、bip324 加密传输、AssumeUTXO、包中继等                          | 完全重写     |
| 钱包功能     | 只有基本收发                              | HD 钱包、descriptor wallet、PSBT、CoinJoin 接口、Taproot 输出等       | 完全不同     |
| 节点实现语言 | C++（但结构非常原始）                     | 现代 C++17/20，模块化，单元测试覆盖率 >95%                            | 代码重构多次 |
| 关键激活功能 | 无                                        | SegWit（2017.08）、Taproot（2021.11）、CSV、BIP-34/66/65 等十几项软分叉 | 全部新增     |
| 挖矿算法     | 只有 CPU 挖矿，GetWork 协议              | Stratum V2、ASICBoost、已废弃 GetWork                                 | 完全不同     |
| 代码行数     | 约 33,000 行                              | 超过 150,000 行（不含测试）                                           | 增长5倍以上  |
